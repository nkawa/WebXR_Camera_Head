...
////    const [sora_render, set_sora_render] = React.useState(false)
    ...
    //ビデオ登録
    React.useEffect(() => {
        console.log("Sora Effect",Sora)
        if (typeof Sora !== 'undefined' && sora_wait  && objectRender) {
            //const signalingUrl = 'wss://sora.uclab.jp/signaling'; //uclab用
            const signalingUrl = 'wss://sora2.uclab.jp/signaling'; //VR_demo用
            const channelId = 'jon';
//            const channelId1 = 'realsense';
//            const channelId2 = 'sora-audio';
            console.log("Now Sora is ready?",Sora)
            const sora = Sora.connection(signalingUrl);
            const bundleId = 'jon-sora-bundle';
...
/*
    ...
    React.useEffect(() => {
        const scene = document.querySelector('a-scene');
        const UIBack = document.querySelector('#rig');
        if (scene && rendered) {
            console.log("add video asset start!");
            //assetの追加
...
//original                    geometry = new THREE.SphereGeometry(geo_def.radius || 100, geo_def.segmentsWidth || 64, geo_def.segmentsHeight || 64, 19 * Math.PI / 36, 17 * Math.PI / 18, 0, Math.PI);
// for Raspberry Cam x2 from Jetson
                    geometry = new THREE.SphereGeometry(geo_def.radius || 1, geo_def.segmentsWidth || 64, geo_def.segmentsHeight || 64, 23 * Math.PI / 36, 13 * Math.PI / 18, 0.95, Math.PI-1.9);
                } else {
...
//                console.log("videoEl",this.videoEl)
                if (this.videoEl != null) {
                    ...
                    this.videoEl.play().catch(error => console.error('Error playing video:', error));
                }else{
                    console.log("videoEl is null")
                }
            } else {
...
if(!('video-play' in AFRAME.components)){
    console.log("Register Video-Play")
