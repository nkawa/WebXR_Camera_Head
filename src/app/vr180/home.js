...
import StereoVideo from '../../lib/stereoVideo.js';
...
//          trigger: [gripRef.current, buttonaRef.current, buttonbRef.current,gripValueRef.current]
    ...
    React.useEffect(() => {  
        if(!registered){
          console.log("Initialize A-Frame components and others")
          setTimeout(()=>{console.log("Set Render!!"); set_rendered(true)},1000)
          registered = true
...
//                      console.log("SetVR_Target ",wpos,targetRef.current);
          ...
          console.log("registered A-Frame components and others")
        }
    },[]); // run only onece
    ...
    if(rendered){
      return (
      <>
        <a-scene scene xr-mode-ui="XRMode: vr">
...
{/*          <Select_Robot {...robotProps}/>*/}
          ...
          <a-entity id="rig" position={`${c_pos_x} ${c_pos_y} ${c_pos_z}`} rotation={`${c_deg_x} ${c_deg_y} ${c_deg_z}`}>
            <a-camera id="camera" cursor="rayOrigin: mouse;" camera-head position="0 0 0" stereocam='eye:left;'></a-camera>
          </a-entity>
          {/*
